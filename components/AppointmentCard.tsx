              ...
              {openMenu && (
                <>
                  <div className="absolute top-full right-0 mt-1 w-48 bg-gradient-to-br from-slate-900 to-slate-950 border border-slate-800 rounded-lg shadow-xl overflow-hidden z-[60]">
                    ...
                    {startTime > now && onStartAppointment && (
                      <button
                        onClick={() => {
                    ...
                    {startTime <= now && (appointment.status === 'PENDING' || appointment.status === 'CONFIRMED' || appointment.status === 'CONFIRMED_BY_CLIENT' || appointment.status === 'NO_SHOW') && onNoShow && (
                      <button
                        onClick={() => {
